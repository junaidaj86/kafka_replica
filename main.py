...
def print_func():
    ...
    _consumer_topic = Topic(
        name="_consumer_topic",
        partition_count=3,
        replication_factor=1,
        retention_ms=60000,
        retention_bytes=1048576,
    ...
    log_store = LogManager(base_path=Path(__file__).parent / "data")
    ...
    for message in messages:
        print(
            f"Consumed message: key={message.key}, value={message.value}, timestamp={message.timestamp}"
        )
