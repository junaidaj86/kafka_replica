...
class Broker:
    ...
    def __init__(self, topic_store: TopicStore, log_store: LogStore):
    ...
    def consume(self, topic_name: str, partition_id: int, offset: int, max_records: int = 100) -> List[Message]:
        topic = self.get_topic(topic_name)
        self._validate_partition(topic, partition_id)
        return self.log_store.read_from_offset(topic_name, partition_id, offset, max_records=max_records)
    ...
    def _validate_partition(self, topic: Topic, partition_id: int)-> None:
        if partition_id < 0 or partition_id >= topic.partition_count:
            raise ValueError(f"Invalid partition ID {partition_id} for topic '{topic.name}'.")
